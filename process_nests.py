...
def count_max_consec_detects(nest_data: pd.DataFrame, date_data: pd.DataFrame) -> int:
    ...
    all_dates = sorted(pd.to_datetime(date_data.loc[0, "Date"]))
    pos = {d: i for i, d in enumerate(all_dates)}
    idxs = sorted(
        pos.get(pd.to_datetime(d))
        for d in nest_data["Date"].unique()
        if pd.to_datetime(d) in pos
    )
...
def process_nests(nest_file, year, site, savedir, min_score=0.3, min_detections=3, min_consec_detects=1):
    ...
    date_data = (
        nests_data.groupby(["Site", "Year"])
        .agg({"Date": lambda x: pd.Series(x).unique().tolist()})
        .reset_index()
    )
    ...
    for target_ind in target_inds:
        ...
        if len(nest_data) >= min_detections or num_consec_detects >= min_consec_detects:
            ...
            summed_scores = (
                nest_data.groupby(["Site", "Year", "target_ind", "label"])["score"]
                .agg(["sum", "count"])
                .reset_index()
            )
            ...
            # Summary stats
            nest_info = (
                nest_data.groupby(["Site", "Year", "target_ind"])
                .agg({
                    "Date": ["min", "max", "count"],
                    "match_xmin": ["mean"],
                    "match_xmax": ["mean"],
            ...
            # Access multiindex columns cleanly
            def g(col, stat):
                return float(nest_info[(col, stat)].values[0])

            xmean = (g("match_xmin", "mean") + g("match_xmax", "mean")) / 2.0
            ymean = (g("match_ymin", "mean") + g("match_ymax", "mean")) / 2.0
    ...
    if nests_rows:
        ...
        nests_df = pd.DataFrame(nests_rows, columns=cols)
        nests_gdf = geopandas.GeoDataFrame(
            nests_df,
            geometry=geopandas.points_from_xy(nests_df.xmean, nests_df.ymean),
            crs=nests_data.crs,
        )
        nests_gdf.to_file(filename, driver="ESRI Shapefile", engine="fiona")
    else:
        # Write an empty shapefile with explicit schema using Fiona engine
        schema = {
            "geometry": "Point",
            "properties": {
                "nest_id": "int",
        ...
        empty = geopandas.GeoDataFrame(
            {k: pd.Series(dtype="object") for k in schema["properties"].keys()},
            geometry=geopandas.GeoSeries([], dtype="geometry"),
            crs=nests_data.crs,
        )
        empty.to_file(filename, driver="ESRI Shapefile", schema=schema, engine="fiona")
